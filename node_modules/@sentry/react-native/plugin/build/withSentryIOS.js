...
exports.modifyAppDelegate = exports.removeDisableAutoUploadFromExistingScript = exports.addDisableAutoUploadToExistingScript = exports.addSentryWithBundledScriptsToBundleShellScript = exports.modifyExistingXcodeBuildScript = exports.withSentryIOS = void 0;
...
const withSentryIOS = (config, { sentryProperties, useNativeInit = false, disableAutoUpload = false }) => {
...
function modifyExistingXcodeBuildScript(script, disableAutoUpload = false) {
...
function addSentryWithBundledScriptsToBundleShellScript(script, disableAutoUpload = false) {
    const disableAutoUploadExport = disableAutoUpload ? `${SENTRY_DISABLE_AUTO_UPLOAD_EXPORT}\n` : '';
    return script.replace(/^.*?(packager|scripts)\/react-native-xcode\.sh\s*(\\'\\\\")?/m, (match) => `${disableAutoUploadExport}/bin/sh ${SENTRY_REACT_NATIVE_XCODE_PATH} ${match}`);
}
exports.addSentryWithBundledScriptsToBundleShellScript = addSentryWithBundledScriptsToBundleShellScript;
function addDisableAutoUploadToExistingScript(script) {
...
exports.addDisableAutoUploadToExistingScript = addDisableAutoUploadToExistingScript;
function insertExportAfterDelimiter(script) {
...
function removeDisableAutoUploadFromExistingScript(script) {
    if (!script.shellScript.includes('SENTRY_DISABLE_AUTO_UPLOAD')) {
        return;
    }
    try {
...
exports.removeDisableAutoUploadFromExistingScript = removeDisableAutoUploadFromExistingScript;
